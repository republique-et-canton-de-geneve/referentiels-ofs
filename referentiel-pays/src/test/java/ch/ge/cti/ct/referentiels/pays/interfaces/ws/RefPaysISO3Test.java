...
public class RefPaysISO3Test extends AbstractRefWSTest {
    ...
    public RefPaysISO3Test(final String iso2, final int continentId,
	...
	final PaysWS pays = getWS().getPaysByIso3(iso3);

	if (paysNom == null) {
	    assertNull("Pays[" + iso3 + "] est incorrect", pays);
	} else {
	    ...
	    assertEquals("Pays[" + iso3 + "].iso3 est incorrect", iso3,
		    pays.getIso3());
	    assertEquals("Pays[" + iso3 + "].nom est incorrect", paysNom,
		    pays.getNom());
	    assertEquals("Pays[" + iso3 + "].nomLong est incorrect",
