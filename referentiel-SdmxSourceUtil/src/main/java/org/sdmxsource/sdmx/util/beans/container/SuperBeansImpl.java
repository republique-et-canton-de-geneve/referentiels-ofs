...
public class SuperBeansImpl implements SuperBeans{
		...
		if(allBeans != null) {
			for(MaintainableSuperBean currentBean : allBeans) {
				if(currentBean instanceof CategorySchemeSuperBean) {
					categorySchemes.add((CategorySchemeSuperBean)currentBean);
				} else if(currentBean instanceof CodelistSuperBean) {
					codelists.add((CodelistSuperBean)currentBean);
				} else if(currentBean instanceof ConceptSchemeSuperBean) {
					conceptSchemes.add((ConceptSchemeSuperBean)currentBean);
				} else if(currentBean instanceof DataflowSuperBean) {
					dataflows.add((DataflowSuperBean)currentBean);
				} else if(currentBean instanceof HierarchicalCodelistSuperBean) {
					hcls.add((HierarchicalCodelistSuperBean)currentBean);
				} else if(currentBean instanceof DataStructureSuperBean) {
					dataStructures.add((DataStructureSuperBean)currentBean);
				} else if(currentBean.getBuiltFrom().getStructureType() == SDMX_STRUCTURE_TYPE.PROVISION_AGREEMENT) {
					provisionAgreement.add((ProvisionAgreementSuperBean)currentBean);
				} else if(currentBean instanceof ProcessSuperBean) {
					processes.add((ProcessSuperBean)currentBean);
				} else if(currentBean instanceof RegistrationSuperBean) {
					registrations.add((RegistrationSuperBean)currentBean);
				} 
			}
		}
	}
	...
	public void addMaintainable(MaintainableSuperBean bean) {
		...
		case CATEGORY_SCHEME : addCategoryScheme((CategorySchemeSuperBean)bean);
		break;
		case CODE_LIST : addCodelist((CodelistSuperBean)bean);
		break;
		case CONCEPT_SCHEME : addConceptScheme((ConceptSchemeSuperBean)bean);
		break;
		case DATAFLOW : addDataflow((DataflowSuperBean)bean);
		break;
		case HIERARCHICAL_CODELIST : addHierarchicalCodelist((HierarchicalCodelistSuperBean)bean);
		break;
		case DSD : addDataStructure((DataStructureSuperBean)bean);
		break;
		case PROCESS : addProcess((ProcessSuperBean)bean);
		break;
		case PROVISION_AGREEMENT : addHierarchicalCodelist((HierarchicalCodelistSuperBean)bean);
		break;
